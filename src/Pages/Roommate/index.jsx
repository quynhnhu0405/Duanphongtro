  ...
  useEffect(() => {
    const fetchRoommates = async () => {
      try {
        ...
        // Fetch filtered data
        const response = await postService.getRoommates(queryParams);
        setRoom(response.data);
        setError(null);
      } catch (err) {
        console.error("Lỗi khi tải bài ở ghép:", err);
        setError("Không thể tải danh sách ở ghép");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div>
      <Card className="bg-white p-7">
        <div className="border-b border-gray-200 pb-2">
          <h1 className="font-bold text-2xl">Tìm Người Ở Ghép Mới Nhất 2025</h1>
